...
def get_test_avg_par2(summary_path: Path) -> tuple[float | None, float | None]:
    """Return (mean, std) of test avg_par2 (raw PAR-2 value) from summary.json, or (None, None)."""
    ...
    values = [
        float(s["test_metrics"]["avg_par2"])
        for s in seeds
        if s and "test_metrics" in s and "avg_par2" in s.get("test_metrics", {})
    ]
    if not values:
        return None, None
...
def main() -> None:
    ...
    # Collect data: {(logic, label): (mean, std)} for test avg_par2
    data: dict[tuple[str, str], tuple[float | None, float | None]] = {}
    for res_dir, label in zip(result_dirs, LABELS):
        for logic in logics:
            data[(logic, label)] = get_test_avg_par2(res_dir / logic / "summary.json")
    ...
    for logic in logics:
        ...
        min_val = min(row_vals.values()) if row_vals else None
